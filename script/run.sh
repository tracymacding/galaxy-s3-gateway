...
cd $BINDIR && nohup ./galaxy-s3-gateway -gfs_zk_addr=$zookeeper -log_dir=$log_dir -logtostderr=false -mongodb_addr=$mongodb_address -port=$listen_port -gfs_scheduler_path=$scheduler_zk_path > nohup.out 2>&1 &
